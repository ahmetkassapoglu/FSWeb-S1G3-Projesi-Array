...
function indekstekiCesitiGetir(tat,sayi){
  return tat[sayi]
}
...
function ismeGoreCesitCikar(tat,lezzetAdi){
  let index = tat.indexOf(lezzetAdi);
  tat.splice(index, 1)
  return tat
}
...
function ortalamaKelimeSayisi(tatlar){
  let toplam =0
  let bolum = tatlar.length
  for (let i=0; i<tatlar.length; i++){
toplam = toplam+ tatlar[i].length
  }
  return toplam / bolum
}
